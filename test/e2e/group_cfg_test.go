...
func TestSingleGrpNameChng(t *testing.T) {
	...
	groupEndpoint := fmt.Sprintf("http://%s/manage/v2/groups/Default/properties", tunnel.Endpoint())
	responseCode, err := VerifyGrpNameChng(t, groupEndpoint, kubectlOptions, "Default", "newDefault")
	if err != nil {
		t.Fatalf(err.Error())
	}
	if responseCode != 204 {
	...
	groupEndpoint = fmt.Sprintf("http://%s/manage/v2/groups/newDefault/properties?format=json", tunnel.Endpoint())
	request := digestAuth.NewRequest(username, password, "GET", groupEndpoint, "")
...
func TestGroupNameChng(t *testing.T) {
	...
	// change the group name for dnode and verify it passes
	groupEndpoint := fmt.Sprintf("http://%s/manage/v2/groups/dnode/properties", tunnel.Endpoint())
	responseCode, err := VerifyGrpNameChng(t, groupEndpoint, kubectlOptions, "dnode", "newDnode")
	if err != nil {
		t.Fatalf(err.Error())
	}
	if responseCode != 204 {
	...
	groupEndpoint = fmt.Sprintf("http://%s/manage/v2/groups/enode/properties", tunnel.Endpoint())
	responseCode, err = VerifyGrpNameChng(t, groupEndpoint, kubectlOptions, "enode", "newDnode")
	if err != nil {
		t.Fatalf(err.Error())
	}
	...
	groupEndpoint = fmt.Sprintf("http://%s/manage/v2/groups/enode/properties", tunnel.Endpoint())
	responseCode, err = VerifyGrpNameChng(t, groupEndpoint, kubectlOptions, "enode", "newEnode")
	if err != nil {
		t.Fatalf(err.Error())
	}
